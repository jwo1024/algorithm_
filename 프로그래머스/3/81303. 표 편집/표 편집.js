...
function solution(n, k, cmd) {
  ...
  cmd.forEach((command) => {
    if (command[0] === "U") {
      ...
      } else currentIdx = arr[currentIdx].pre;
    } else if (command[0] === "Z" && deleteStack.length) {
      let pop = deleteStack.pop();
      if (pop.pre !== -1) arr[pop.pre].next = pop.idx;
      if (pop.next !== -1) arr[pop.next].pre = pop.idx;
    }
  });
...
function moveCurrentIdxUp(arr, currentIdx, num) {
  for (let i = 0; i < num; i++) {
    if (arr[currentIdx]) currentIdx = arr[currentIdx].pre;
  }
  return currentIdx;
}
...
function moveCurrentIdxDown(arr, currentIdx, num) {
  for (let i = 0; i < num; i++) {
    if (arr[currentIdx]) currentIdx = arr[currentIdx].next;
  }
  return currentIdx;
}
